...
// ============================================================
// 1. [핵심] 사이트 리소스 기반 PPT 생성기 (이미지/데이터 완벽 수정판)
// ============================================================

async function createDynamicPPT() {
    // 1. 라이브러리 로드 체크
    if (typeof window.PptxGenJS === 'undefined') {
        throw new Error("PptxGenJS 라이브러리가 로드되지 않았습니다. (HTML 헤더 확인 필요)");
    }
    ...
    // ⭐ 5. 데이터 구조 평탄화 (에러 해결 핵심 로직!)
    // db가 배열이면 그대로 쓰고, 객체(카테고리별)면 한 줄로 쫙 폅니다.
    let allResources = Array.isArray(db) ? db : Object.values(db).flat();

    // 데이터가 비었는지 확인
    if (!allResources || allResources.length === 0) {
        console.error("데이터가 없습니다. data.js를 확인해주세요.");
        return;
    }

    // 6. 앞에서 4개만 뽑아서 슬라이드 만들기
    ...
    resources.forEach((item, index) => {
        ...
        // 설명
        slide.addText(item.desc || '설명이 없습니다.', { x: 0.5, y: 1.5, w: 4.5, fontSize: 14, color: '475569', lineSpacing: 20 });
        ...
        // 이미지 경로 절대 주소 변환 (안전한 이미지 로드)
        if (item.thumb) {
            try {
                const absolutePath = new URL(item.thumb, window.location.href).href;
                console.log(`슬라이드 ${index+1} 이미지 추가:`, absolutePath); // 디버깅용 로그
                ...
                console.warn(`이미지 경로 변환 실패 (${item.title}):`, err);
            }
        }
    });
    ...
    setTimeout(async () => {
        ...
        if (text.includes("PPT") || text.includes("만들어") || text.includes("슬라이드")) {
            ...
            try {
                ...
                console.error(e);
                typeWriter(`❌ PPT 생성 오류: ${e.message}<br>브라우저 콘솔(F12)을 확인해주세요.`);
            }
            ...
            else if (text.includes("다운") || text.includes("오류")) answer = "다운로드 버튼을 누르시면 구글 드라이브 뷰어가 새 창으로 열립니다.";
