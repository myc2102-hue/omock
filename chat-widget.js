...
async function createDynamicPPT() {
    ...
    // ---------------------------------------------------------
    // 🔧 [수정된 부분] 데이터가 객체여도 배열로 변환해서 처리
    // ---------------------------------------------------------
    let allResources = [];

    if (Array.isArray(db)) {
        // 1. 만약 db가 이미 리스트라면 그대로 사용
        allResources = db;
    } else {
        // 2. 만약 db가 카테고리별(Object)로 되어 있다면 -> 하나로 합침
        // Object.values(db)는 각 카테고리의 내용물들을 꺼내고, .flat()은 그걸 한 줄로 폅니다.
    ...
    resources.forEach((item, index) => {
        ...
        // 설명
        slide.addText(item.desc || 'No description available.', { x: 0.5, y: 1.5, w: 4.5, fontSize: 14, color: '475569', lineSpacing: 20 });
    ...
    setTimeout(async () => {
        ...
        if (text.includes("PPT") || text.includes("만들어") || text.includes("슬라이드")) {
            ...
            try {
                ...
                console.error(e);
                typeWriter(`❌ PPT 생성 오류: ${e.message}<br>브라우저 콘솔을 확인해주세요.`);
            }
