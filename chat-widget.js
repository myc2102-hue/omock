...
// ⚠️ 중요: HTML 헤더에서 스크립트를 불러왔으므로 import 문은 삭제했습니다.
...
async function createDynamicPPT() {
    // ⭐ [수정됨] window.PptxGenJS 로 변경하여 HTML에 로드된 라이브러리를 확실하게 잡습니다.
    if (typeof window.PptxGenJS === 'undefined') {
        throw new Error("PptxGenJS 라이브러리가 로드되지 않았습니다.");
    }
    
    let pptx = new window.PptxGenJS(); // window. 을 붙여야 안전합니다.
    ...
    setTimeout(async () => {
        ...
        if (text.includes("PPT") || text.includes("만들어") || text.includes("슬라이드")) {
            ...
            try {
                ...
                console.error(e);
                typeWriter("❌ PPT 생성 중 오류가 발생했습니다.<br>HTML 헤더에 스크립트는 잘 넣으셨네요!<br>브라우저 콘솔(F12)을 확인해주세요.");
            }
